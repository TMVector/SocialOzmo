...
and [<AllowNullLiteral>] [<Import("*","ErrorResponse")>] ErrorResponse(xhr: XMLHttpRequest) =
...
and [<AllowNullLiteral>] [<Import("*","Subscription")>] Subscription(xhr: XMLHttpRequest, options: SubscribeOptions) =
...
and [<AllowNullLiteral>] [<Import("*","BaseClient")>] BaseClient(options: BaseClientOptions) =
...
and [<AllowNullLiteral>] [<Import("*","SimpleTokenAuthorizer")>] SimpleTokenAuthorizer(jwt: string) =
    interface Authorizer with
        member __.authorize(): Promise<string> = jsNative


and [<AllowNullLiteral>] [<Import("*","AuthServerAuthorizer")>] AuthServerAuthorizer(authServerUrl: string) =
    interface Authorizer with
        member __.authorize(): Promise<string> = jsNative
...
and [<AllowNullLiteral>] [<Import("*","FeedsHelper")>] FeedsHelper(name: string, app: App) =
...
and [<AllowNullLiteral>] [<Import("*","PusherPlatform.App")>] App(options: AppOptions) =
